...
const AllTickets =  () => {
  ...
  return (
    <div className='flex flex-col gap-5 items-center  mt-10'>
        ...
        <div className='text-3xl  text-sky-900'>Explore Tickets</div>
        <hr className='text-slate-300'/>
        </div>
   
    <div className='flex flex-wrap gap-5 items-center justify-center sm:p-10 p-4'>
      {tickets?.map((ticket:Record<string,any>) =>  {
        return (
        ...
        <div className=" text-6xl">
          📃
            ...
            <div className='flex flex-col gap-5'>
                <div className='text-xl text-slate-700 '>{ticket?.title && ticket.title.charAt(0).toUpperCase() + ticket.title.slice(1) }</div>
                <hr className='text-slate-300'/>
